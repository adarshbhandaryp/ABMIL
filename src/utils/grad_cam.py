...
def superimpose_to_mask(input_img, heatmap):
    img = cv2.cvtColor(input_img,cv2.COLOR_BGR2RGB)
    heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
    heatmap = np.uint8(255 * heatmap)
    heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_BONE)
    superimposed_img = np.uint8(heatmap * 1 + img * 0.0)
    pil_img = cv2.cvtColor(heatmap,cv2.COLOR_BGR2GRAY)
    (thresh, blackAndWhiteImage) = cv2.threshold(pil_img, 127, 255, cv2.THRESH_BINARY)
...
def save_grad_grad_cam(model, input_tensor, heatmap_layer, truelabel=None):
    info = InfoHolder(heatmap_layer)
    heatmap_layer.register_forward_hook(info.hook)
    ...
    heatmap = generate_heatmap(weighted_activation)
    input_image = to_RGB(input_tensor)
    grad_image = superimpose_to_mask(input_image, heatmap)
    return input_image, grad_image